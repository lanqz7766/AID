...
class BaseCAM:
    ...
    def get_cam_image(self,
        ...
        # logger.info(f'weighted_activations is: {weighted_activations.shape}')
        if eigen_smooth:
    ...
    def forward(self,
        ...
        if self.cuda:
            ...
            # logger.info(f'input_tensor shape is : {input_tensor.shape}')
        ...
        cam_per_layer = self.compute_cam_per_layer(input_tensor,
                                                   targets,
                                                   eigen_smooth)
        # logger.info(f"cam_per_layer shape is: {cam_per_layer[0].shape}" )
        # logger.info(f"cam_per_layer shape after aggregate: {self.aggregate_multi_layers(cam_per_layer).shape}")
        # return self.aggregate_multi_layers(cam_per_layer) # (1, 375, 1242)
        return cam_per_layer # (1, 1, 375, 1242)
    ...
    def compute_cam_per_layer(
        ...
        # logger.info(f"length of target_layers: {len(self.target_layers)}")
        ...
        for i in range(len(self.target_layers)):
            ...
            cam = np.maximum(cam, 0) # (1, 20, 20)

            scaled = scale_cam_image(cam, target_size=None) #
            # logger.info(f'After scaled(cam): {scaled.shape}')
            # cam_per_target_layer.append(scaled[:, None, :])
            cam_per_target_layer.append(scaled)
        # cam_per_target_layer = np.asarray(cam_per_target_layer)
            # logger.info(f'cam_per_target_layer after scaled(cam): {len(cam_per_target_layer), cam_per_target_layer[0].shape}')
...
class DetCAMVisualizer:
    ...
    def __call__(self, img, targets, aug_smooth=False, eigen_smooth=False):
        ...
        # logger.info(f'img.shape after permute(0,3,1,2): {img.shape}')
        # gray_cam = self.cam(img, targets, aug_smooth, eigen_smooth)
        # logger.info(f'gray_cam shape: {len(gray_cam)}')
    ...
    def show_cam(self,
        ...
        logger.info(f'grayscale_cam before scaled shape is: {grayscale_cam.shape}')
        grayscale_cam = np.transpose(scale_cam_image(grayscale_cam, target_size), (1, 2, 0))
        logger.info(f'grayscale_cam after scaled shape is: {grayscale_cam.shape}')
