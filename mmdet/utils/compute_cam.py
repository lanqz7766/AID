...
from mmdet.utils.basecam import BaseCAM, scale_cam_image          ##################
...
#        self.COLORS = np.random.uniform(0, 255, size=(len(self.classes), 3))
        self.COLORS = [[50,205,50], [255, 255, 0], [0, 110, 255]]          ####################
...
#        print(f'\nimg.shape after permute(0,3,1,2): {img.shape}')         ######################
...
#                print(f'\n\nrenormalized_cam max: {renormalized_cam}')
                renormalized_cam = scale_cam_image(renormalized_cam)
#                print(f'\n\nrenormalized_cam scaled size: {renormalized_cam.shape}')
                areas = np.array(areas)
#                print(f'\n\nareas size: {areas.shape}')
                sum_areas = np.sum(areas)
#                print(f'\n\nsum_areas size: {sum_areas.shape}')
            except ValueError:  #####################
                pass
        else:
        ...
        return image_with_bounding_boxes, renormalized_cam/sum_areas       #######################
#        return image_with_bounding_boxes


    def weighted_show_cam(self,
                 image,
                 gt_boxes,
                 boxes,
                 labels,
        ...
        for i, (x1, y1, x2, y2) in enumerate(gt_boxes):
            img = renormalized_cam * 0
            new_x1, new_y1, new_x2, new_y2 = max(x1-margin, 0), max(y1-margin, 0),\
                                             min(x2+margin, image.shape[1]), min(y2+margin, image.shape[0])
            try:
                new_img_cam = scale_cam_image(grayscale_cam[new_y1:new_y2, new_x1:new_x2].copy())
                img[new_y1:new_y2, new_x1:new_x2] = mask_decay[i] * new_img_cam 
 
                images.append(img)
                areas.append((new_y2-new_y1)*(new_x2-new_x1))       ##########
            except ValueError:  #raised if 'img' is empty
                print("\n\nimg is empty")
                pass
