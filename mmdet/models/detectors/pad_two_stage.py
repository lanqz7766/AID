...
class PADTwoStage(TwoStageDetector):
...
#            teacher_x = self.teacher_model.extract_feat(img)
        ...
        if t_feats is not None:
            for _i in range(len(t_feats)):
                log_variances = torch.log(torch.var(self.adaptation_layers[_i](x[_i]),unbiased = False))
                ...
                kd_feat_loss += (dist2(t_feats[_i], self.adaptation_layers[_i](x[_i]), attention_mask=sum_attention_mask,
                                      channel_attention_mask=c_sum_attention_mask)/(1e-8+torch.exp(log_variances)) + log_variances) * 4e-5 #7e-5
                kd_channel_loss += (torch.dist(torch.mean(t_feats[_i], [2, 3]),
                                              self.channel_wise_adaptation[_i](torch.mean(x[_i], [2, 3])))/(1e-8+torch.exp(log_variances)) + log_variances) * 4e-3*2 #4e-3
                ...
                s_spatial_pool = torch.mean(x[_i], [1]).view(x[_i].size(0), 1, x[_i].size(2),
                                                             x[_i].size(3))
                kd_spatial_loss += (torch.dist(t_spatial_pool, self.spatial_wise_adaptation[_i](s_spatial_pool))/(1e-8+torch.exp(log_variances)) + log_variances) * 4e-3*2 #4e-3
        ...
        if t_feats is not None:
            for _i in range(len(t_feats)):
                ...
                kd_nonlocal_loss += torch.dist(self.non_local_adaptation[_i](s_relation), t_relation, p=2)/(1e-8+torch.exp(log_variances)) + log_variances
