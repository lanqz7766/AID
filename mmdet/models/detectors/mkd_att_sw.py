...
class MKDAttentionSW(SingleStageDetector):
    ...
    def __init__(self,
        ...
        self.teacher_model_1 = build_detector(teacher_config_1['model']).cuda()
        
        if isinstance(teacher_config_2, str):
            teacher_config_2 = mmcv.Config.fromfile(teacher_config_2)
        self.teacher_model_2 = build_detector(teacher_config_2['model']).cuda()       
...
#             with torch.no_grad():
#                 teacher_x_1 = self.teacher_model_1.extract_feat(img[i])
#                 losses_T_1 = self.teacher_model_1.bbox_head.forward_train(teacher_x_1, img_metas[i], gt_bboxes[i], gt_labels[i], gt_bboxes_ignore)
#                 T_total_1 = {k: sum(losses_T_1[k]) for k in losses_T_1.keys()}
            
#             with torch.no_grad():
#                 teacher_x_2 = self.teacher_model_2.extract_feat(img[i])
#                 #out_teacher_2 = self.teacher_model_2.bbox_head(teacher_x_2)
#                 losses_T_2 = self.teacher_model_2.bbox_head.forward_train(teacher_x_2, img_metas[i], gt_bboxes[i], gt_labels[i], gt_bboxes_ignore)
#                 T_total_2 = {k: sum(losses_T_2[k]) for k in losses_T_2.keys()}
            
#             # Loss_1 = list(T_total_1.values())[0] + list(T_total_1.values())[1]
#             # Loss_2 = list(T_total_2.values())[0] + list(T_total_2.values())[1]
        
        
        
#             losses = self.bbox_head.forward_train(x[i], img_metas[i],
...
#                     w = torch.exp(-0.1*(losses_T_1['loss_cls'][_i] + losses_T_1['loss_bbox'][_i]))
...
#                                                                  x[_i].size(3))
#                     kd_spatial_loss += w*torch.dist(t_spatial_pool,
...
#             c_s_ratio = 1.0            
#             with torch.no_grad():
#                 t_feats = self.teacher_model_2.extract_feat(img)
               
#             if t_feats is not None:
#                 for _i in range(len(t_feats)):
#                     w = torch.exp(-0.1*(losses_T_2['loss_cls'][_i] + losses_T_2['loss_bbox'][_i]))
#                     t_attention_mask = torch.mean(torch.abs(t_feats[_i]), [1], keepdim=True)
#                     size = t_attention_mask.size()
#                     t_attention_mask = t_attention_mask.view(x[0].size(0), -1)
#                     t_attention_mask = torch.softmax(t_attention_mask / t, dim=1) * size[-1] * size[-2]
...
#             c_s_ratio = 1.0            
#             with torch.no_grad():
#                 t_feats = self.teacher_model_3.extract_feat(img)
               
#             if t_feats is not None:
#                 for _i in range(len(t_feats)):
#                     w = torch.exp(-0.1*(losses_T_3['loss_cls'][_i] + losses_T_3['loss_bbox'][_i]))
#                     t_attention_mask = torch.mean(torch.abs(t_feats[_i]), [1], keepdim=True)
#                     size = t_attention_mask.size()
#                     t_attention_mask = t_attention_mask.view(x[0].size(0), -1)
#                     t_attention_mask = torch.softmax(t_attention_mask / t, dim=1) * size[-1] * size[-2]
...
#             losses.update(kd_nonlocal_loss=kd_nonlocal_loss * 7e-5 )# * 6
    ...
    def __setattr__(self, name, value):
        ...
        """
        if name == 'teacher_model_1' or name == 'teacher_model_2' or name == 'teacher_model_3':
