...
class SingleStagePAD(SingleStageDetector):
...
#                                              gt_bboxes_ignore)
        ...
        if t_feats is not None:
            for _i in range(len(t_feats)):
                ...
                kd_feat_loss += (dist2(t_feats[_i], self.adaptation_layers[_i](x[_i]), attention_mask=sum_attention_mask,
                                          channel_attention_mask=c_sum_attention_mask)/(1e-6+torch.exp(log_variances)) + log_variances) * 7e-5*6 #7e-5 * 6
                kd_channel_loss += (torch.dist(torch.mean(t_feats[_i], [2, 3]),
                                              self.channel_wise_adaptation[_i](torch.mean(x[_i], [2, 3])))/(1e-6+torch.exp(log_variances)) + log_variances) * 4e-3 * 6 #4e-3 * 6
                ...
                kd_spatial_loss += (torch.dist(t_spatial_pool,
                                              self.spatial_wise_adaptation[_i](s_spatial_pool))/(1e-6+torch.exp(log_variances)) + log_variances) * 4e-3 * 6 #4e-3 * 6
        ...
        if t_feats is not None:
            for _i in range(len(t_feats)):
                ...
                kd_nonlocal_loss += (torch.dist(self.non_local_adaptation[_i](s_relation), t_relation, p=2)/(1e-6+torch.exp(log_variances)) + log_variances) * 7e-5*6
