...
class KDTwoStageATTDetector(TwoStageDetector):
...
#            teacher_x = self.teacher_model.extract_feat(img)
        ...
        if t_feats is not None:
            for _i in range(len(t_feats)):
                ...
                kd_channel_loss += torch.dist(torch.mean(t_feats[_i], [2, 3]),
                                              self.channel_wise_adaptation[_i](torch.mean(x[_i], [2, 3]))) * 2e-3 #4e-3
                ...
                s_spatial_pool = torch.mean(x[_i], [1]).view(x[_i].size(0), 1, x[_i].size(2),
                                                             x[_i].size(3))
                kd_spatial_loss += torch.dist(t_spatial_pool, self.spatial_wise_adaptation[_i](s_spatial_pool)) * 2e-3 #4e-3
        ...
        losses.update(kd_nonlocal_loss=kd_nonlocal_loss * 2e-5) #7e-5
