...
class MKnowledgeDistillationSingleStageDetector(SingleStageDetector):
...
#                 teacher_ckpt_3=None,
        ...
        self.teacher_model_1.cuda()
        self.teacher_model_2.cuda()
    def forward_train(self,
        ...
        with torch.no_grad():
            ...
            losses_T_1 = self.teacher_model_1.bbox_head.forward_train(teacher_x_1, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore)
            out_teacher_1 = self.teacher_model_1.bbox_head(teacher_x_1)

        with torch.no_grad():
            teacher_x_2 = self.teacher_model_2.extract_feat(img)
            losses_T_2 = self.teacher_model_2.bbox_head.forward_train(teacher_x_2, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore)
        ...
        #     cls_score_1 = out_teacher_1[0][i]
        #     bbox_pred_1 = out_teacher_1[1][i]
        #     cls_score_2 = out_teacher_2[0][i]
        #     bbox_pred_2 = out_teacher_2[1][i]
        #     cls_score_3 = out_teacher_3[0][i]
        #     bbox_pred_3 = out_teacher_3[1][i]
        ...
        #     # w1 = torch.exp(-0.1*T_loss_1)/(torch.exp(-0.1*T_loss_1)+torch.exp(-0.1*T_loss_2)+torch.exp(-0.1*T_loss_3))
        #     # w2 = torch.exp(-0.1*T_loss_2)/(torch.exp(-0.1*T_loss_1)+torch.exp(-0.1*T_loss_2)+torch.exp(-0.1*T_loss_3))
        #     # w3 = torch.exp(-0.1*T_loss_3)/(torch.exp(-0.1*T_loss_1)+torch.exp(-0.1*T_loss_2)+torch.exp(-0.1*T_loss_3))
        #     out_teacher_1[0][i] = (cls_score_1+cls_score_2+cls_score_3)/3
        #     out_teacher_1[1][i] = (bbox_pred_1+bbox_pred_2+bbox_pred_3)/3
        ...
        T_total_1 = sum(losses_T_1['loss_cls']) + sum(losses_T_1['loss_bbox'])
        T_total_2 = sum(losses_T_2['loss_cls']) + sum(losses_T_2['loss_bbox'])
        # T_total_3 = sum(losses_T_3['loss_cls']) + sum(losses_T_3['loss_bbox'])
        w1 = torch.exp(-T_total_1)/(torch.exp(-T_total_1)+torch.exp(-T_total_2))
        w2 = torch.exp(-T_total_2)/(torch.exp(-T_total_1)+torch.exp(-T_total_2))
        # w3 = torch.exp(-T_total_3)/(torch.exp(-T_total_1)+torch.exp(-T_total_2)+torch.exp(-T_total_3))
        loss_ld_2 = 0
#        loss_ld_3 = 0
        loss_kd_2 = 0
#        loss_kd_3 = 0
        for i in range(len(x)):
            losses['loss_ld'][i] = w1*torch.exp(-0.1*(losses_T_1['loss_bbox'][i])) * losses['loss_ld'][i]
            loss_ld_2 += w2*torch.exp(-0.1*(losses_T_2['loss_bbox'][i]))*losses_2['loss_ld'][i]
#            loss_ld_3 += w3*torch.exp(-0.1*(losses_T_2['loss_bbox'][i]))*losses_3['loss_ld'][i]
            
            losses['loss_kd'][i] = w1*torch.exp(-0.1*(losses_T_1['loss_cls'][i])) * losses['loss_kd'][i]
            loss_kd_2 += w2*torch.exp(-0.1*(losses_T_2['loss_cls'][i]))*losses_2['loss_kd'][i]
#            loss_kd_3 += w2*torch.exp(-0.1*(losses_T_3['loss_cls'][i]))*losses_3['loss_kd'][i]
        losses.update({'loss_ld_2': loss_ld_2})
#        losses.update({'loss_ld_3': loss_ld_3})
        losses.update({'loss_kd_2': loss_kd_2})
#        losses.update({'loss_kd_3': loss_kd_3})
        return losses
