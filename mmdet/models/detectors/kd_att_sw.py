...
class KDAttentionSW(SingleStageDetector):
...
#                                                      img_metas, gt_bboxes_ignore)
        ...
        if t_feats is not None:
            for _i in range(len(t_feats)):
                ...
                kd_feat_loss += w*dist2(t_feats[_i], self.adaptation_layers[_i](x[_i]), attention_mask=sum_attention_mask,
                                      channel_attention_mask=c_sum_attention_mask) * 7e-5*8 #7e-5 * 6
        ...
        losses.update(kd_nonlocal_loss=kd_nonlocal_loss * 7e-5*7) # 7e-5 * 6
