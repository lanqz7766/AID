...
class Gmask_cam_DetectionDistiller(BaseDetector):
    ...
    def __init__(self,
        ...
        super(Gmask_cam_DetectionDistiller, self).__init__()
    ...
    def forward_train(self, img, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore=None, **kwargs):
        ...
        for image_path in images:
            ...
            # image_with_bounding_boxes = self.det_cam_visualizer.show_cam(
                # image, bboxes, labels, grayscale_cam[0], with_norm_in_bboxes=True)
            # mmcv.mkdir_or_exist('../feature_map/test/')
            # out_file = os.path.join('../feature_map/test/', os.path.basename(image_path))
        ...
        for i in range(0,5):
            teacher_cam_mask_per_layer.append(torch.tensor([item[4-i] for item in teacher_cam_masks]))
            student_cam_mask_per_layer.append(torch.tensor([item[4-i] for item in student_cam_masks]))
        ...
        for i, item_loc in enumerate(self.distill_cfg):
            ...
            for item_loss in item_loc.methods:
                ...
                student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat, teacher_cam_mask_per_layer[i], student_cam_mask_per_layer[i], gt_bboxes, img_metas)
