...
class AID_DetectionDistiller_New(BaseDetector):
    ...
    def __init__(self,
        ...
        super(AID_DetectionDistiller_New, self).__init__()
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        for i in range(len(img)):
            with torch.no_grad():
                self.teacher.eval()
                feat = self.teacher.extract_feat([img[i]])
                losses_T = self.teacher.forward_train([img[i]], [img_metas[i]], **kwargs)
               
            student_loss = self.student.forward_train([img[i]], [img_metas[i]], **kwargs)
            ...
            for item_loc in self.distill_cfg:
                ...
                for item_loss, _i in zip(item_loc.methods, range(0,4)):
                    w = torch.exp(-0.1*(losses_T['loss_cls'][_i] + losses_T['loss_bbox'][_i]))
                    loss_name = item_loss.name
                    
                    student_loss[loss_name] += w * self.distill_losses[loss_name](student_feat,teacher_feat, kwargs['gt_bboxes'][i], img_metas[i])
