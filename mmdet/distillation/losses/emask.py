...
class Emask_FeatureLoss(nn.Module):
    ...
    def __init__(self,
        ...
        super(Emask_FeatureLoss, self).__init__()
        self.temp = temp
        ...
        for i in range(N):
            ...
            for j in range(len(gt_bboxes[i])):
                ...
                big_map[i][2*hmin[i][j]:2*hmax[i][j], 2*wmin[i][j]:2*wmax[i][j]] = \
                        torch.maximum(big_map[i][2*hmin[i][j]:2*hmax[i][j], 2*wmin[i][j]:2*wmax[i][j]], self.calculate_exponential(w, h))
    ...
    def calculate_exponential(self, old_w, old_h):
        w = 2*old_w
        h = 2*old_h
        cx = (w/2).int()
        cy = (h/2).int()
